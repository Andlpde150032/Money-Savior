...
export function ExpenseLineChart({ dateRange }: ExpenseLineChartProps) {
  ...
  const cumulativeData = days.map((day) => {
    ...
    return {
      date: day,
      "Tổng chi tiêu": cumulativeExpense,
      "Tổng thu nhập": cumulativeIncome,
      "Số dư": cumulativeIncome - cumulativeExpense,
    }
  })
  ...
  const data = cumulativeData.map((day) => ({
    ...
    "Tổng chi tiêu": day["Tổng chi tiêu"],
    "Tổng thu nhập": day["Tổng thu nhập"],
    "Số dư": day["Số dư"],
  }))
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={data}>
        ...
        <Line type="monotone" dataKey="Tổng chi tiêu" stroke="#ef4444" />
        <Line type="monotone" dataKey="Tổng thu nhập" stroke="#10b981" />
        <Line type="monotone" dataKey="Số dư" stroke="#3b82f6" />
      </LineChart>
    </ResponsiveContainer>
  )
}
